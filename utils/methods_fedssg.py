...
def p(t, gamma, com_amount, alpha_coef):
    y = alpha_coef * (math.exp((t/com_amount)**gamma) - 1)
    # y = (1/math.pi)*(math.atan(gamma*((t/com_amount)-0.5))+(math.pi/2))#gamma=40->x\in[-20.20]
    # y = abs(math.sin(math.pi*gamma*t/com_amount))
    return y

def train_FedSSG(data_obj, act_prob,n_minibatch,
    ...
    suffix = ('FedSSG') + str(alpha_coef) + suffix
    ...
    writer = SummaryWriter('%sRuns/%s/%s' %(data_path, data_obj.name, suffix[:30]))
    ...
    if (trial) or (not os.path.exists('%sModel/%s/%s/ins_avg_%dcom.pt' %(data_path, data_obj.name, suffix, com_amount))):
        ...
        for i in range(saved_itr+1, com_amount):
            ...
            # p_value = p(i, gamma_p, com_amount, alpha_coef)
            # p_value = torch.zeros(com_amount, dtype=torch.float32)
            for clnt in selected_clnts:
                ...
                # h_mean = hist_i / (i + 1)
                # # h_mean_n = h_mean / torch.max(h_mean)
                # p_temp = torch.sum(h_mean * h_mean)
                # # print(p_temp)
                # p_value[i] = p_temp
                # if i == 0 or p_value[i-1] == 0:
                #     p = p_value[i]
                # else:
                #     p = p_value[i] / p_value[i-1]
                # # print(p)

                # p_value = (count[clnt] / (com_amount * act_prob)) + (i/com_amount) ** (1/2)
                p_value = (count[clnt] / (com_amount * act_prob)) + 1
                clnt_models[clnt] = train_model_FedLDC(model, model_func, alpha,local_update_last, global_update_last,global_mdl, hist_i,
                ...
                delta_param_curr = (curr_model_par - cld_mdl_param) * p_value.numpy()
                # delta_param_curr = (curr_model_par - cld_mdl_param)
                parameter_drifts[clnt] += delta_param_curr
                # p_value = torch.sum(torch.tensor(delta_param_curr) * torch.tensor(delta_param_curr)) / torch.sum(h_mean * h_mean)
