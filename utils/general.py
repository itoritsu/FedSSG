...
def train_model_FedDC_KD(model, cur_cld_model,model_func, alpha,  global_model_param, hist_i,
                      trn_x, trn_y,
                      learning_rate, batch_size, epoch, kd_T,print_per,
                      weight_decay, dataset_name, sch_step, sch_gamma):
    n_trn = trn_x.shape[0]
    ...
    for e in range(epoch):
        ...
        for i in range(int(np.ceil(n_trn / batch_size))):
            ...
            loss_f_i = loss_f_i / list(batch_y.size())[0]
            loss_k_i = loss_div(y_pred, y_t)
            loss_k_d = loss_kd(feat_s[-1], feat_t[-1])
            ...
            loss = loss_f_i + loss_cp + loss_k_i + loss_k_d
            optimizer.zero_grad()
            loss.backward()
            torch.nn.utils.clip_grad_norm_(parameters=model.parameters(),
                                           max_norm=max_norm)  # Clip gradients to prevent exploding
...
def train_model_FedFast(model, cur_cld_model,model_func, alpha,  global_model_param, hist_i,
                      trn_x, trn_y,
                      learning_rate, batch_size, epoch, kd_T,print_per,
                      weight_decay, dataset_name, sch_step, sch_gamma):
    n_trn = trn_x.shape[0]
...
def train_model_FedLDC(model, model_func, alpha, local_update_last, global_update_last, global_model_param, hist_i,
                      trn_x, trn_y,
                      learning_rate, batch_size, epoch, print_per,
                      weight_decay, dataset_name, sch_step, sch_gamma):
    n_trn = trn_x.shape[0]
    ...
    for e in range(epoch):
        ...
        for i in range(int(np.ceil(n_trn / batch_size))):
            ...
            loss_cp = alpha * torch.sum((hist_i) * (local_parameter - (global_model_param - hist_i)))
            loss_cg = torch.sum(local_parameter * state_update_diff)
...
def train_model_FedLDC_localloss(model, model_func, alpha, local_update_last, global_update_last, global_model_param, hist_i,
                      trn_x, trn_y,
                      learning_rate, batch_size, epoch, print_per,
                      weight_decay, dataset_name, sch_step, sch_gamma):
    n_trn = trn_x.shape[0]
...
def train_model_localloss(model, trn_x, trn_y, tst_x, tst_y, learning_rate, batch_size, epoch, print_per, weight_decay, dataset_name, sch_step=1, sch_gamma=1):
    n_trn = trn_x.shape[0]
    trn_gen = data.DataLoader(Dataset(trn_x, trn_y, train=True, dataset_name=dataset_name), batch_size=batch_size, shuffle=True)
    loss_fn = torch.nn.CrossEntropyLoss(reduction='sum')
