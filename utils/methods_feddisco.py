...
def train_FedDisco(data_obj, act_prob,
    ...
    writer = SummaryWriter('%sRuns/%s/%s' % (data_path, data_obj.name, suffix[:28]))
    ...
    not os.path.exists('%sModel/%s/%s/ins_avg_%dcom.pt' % (data_path, data_obj.name, suffix, com_amount))):
        ...
        for i in range(saved_itr + 1, com_amount):
            ...
            # acc_class_tst, acc_tst = get_acc_classwise(data_obj.tst_x,data_obj.tst_y,all_model,data_obj.dataset,0,10)
            # print("**** Cur All Communication %3d, Test class Accuracy: %.4f, Accuracy: %.4f"
            #       % (i + 1, acc_class_tst, acc_tst))
